...
const Plantillas = ({ onApplyTemplate }) => {
  ...
  const cargarPlantillas = async () => {
    setCargando(true);
    try {
      const response = await fetch(`${API_URL}/api/newsletter/plantillas`);
  ...
  const eliminarPlantilla = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar esta plantilla?')) {
      try {
        await fetch(`${API_URL}/api/newsletter/plantillas/${id}`, {
          method: 'DELETE'
        });
  ...
  const guardarPlantilla = async (e) => {
    ...
    try {
      const url = plantillaEditando
        ? `${API_URL}/api/newsletter/plantillas/${plantillaEditando.id}`
        : `${API_URL}/api/newsletter/plantillas`;
